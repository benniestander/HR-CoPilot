                ...
                <div className="p-8 overflow-y-auto space-y-6 text-sm relative">
                    ...
                    <div className="p-4 border rounded-lg grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <DetailRow label="Current Plan" value={<span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${activeUser.plan === 'agency' ? 'bg-indigo-100 text-indigo-800' : activeUser.plan === 'consultant' ? 'bg-orange-100 text-orange-800' : activeUser.plan === 'pro' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}>{activeUser.plan.toUpperCase()}</span>} />

                            {/* Standard Upsells */}
                            {activeUser.plan !== 'agency' && activeUser.plan !== 'consultant' && (
                                <>
                            ...
                            {(activeUser.plan === 'consultant' || activeUser.plan === 'agency') && (
                                <div className="mt-4 pt-4 border-t border-gray-100">
                                    <h4 className="text-xs font-black text-gray-400 uppercase tracking-widest mb-2">Consultant Ops</h4>

                                    <div className="bg-slate-50 p-3 rounded-lg mb-3">
                                        <div className="flex justify-between text-xs mb-1">
                                            <span className="font-bold text-gray-600">Clients:</span>
                                            <span className="font-mono">{activeUser.clients?.length || 0}</span>
                                        </div>
                                        <div className="flex justify-between text-xs">
                                            <span className="font-bold text-gray-600">Platform Access:</span>
                                            <span className={activeUser.consultantPlatformFeePaidUntil && new Date(activeUser.consultantPlatformFeePaidUntil) > new Date() ? "text-green-600 font-bold" : "text-red-500 font-bold"}>
                                                {activeUser.consultantPlatformFeePaidUntil && new Date(activeUser.consultantPlatformFeePaidUntil) > new Date() ? 'Active' : 'Locked'}
                                            </span>
                                        </div>
                                    </div>

                                    {activeUser.plan === 'consultant' && (
                                        <button
                                            onClick={async () => {
                                                if (confirm("Upgrade this user to Agency Tier (R5,000 value)? This overrides manual billing.")) {
                                                    // Call admin upgrade function (needs to be added to adminActions interface, using generic update for now or adjustCredit as hack)
                                                    // Ideally: adminActions.upgradeToAgency(activeUser.uid)
                                                    alert("Please use the database CLI to force upgrade for now, or manually add R5000 credit and simulate payment.");
                                                }
                                            }}
                                            className="w-full px-4 py-2 text-xs font-bold text-white bg-indigo-600 rounded-md hover:bg-indigo-700 flex justify-center items-center mb-2"
                                        >
                                            Force Upgrade to Agency
                                        </button>
                                    )}

                                    <button
                                        onClick={async () => {
                                            if (confirm("Extend Platform Access by 1 year (Free Grant)?")) {
                                                const nextYear = new Date();
                                                nextYear.setFullYear(nextYear.getFullYear() + 1);
                                                await adminActions.updateUser(activeUser.uid, { consultantPlatformFeePaidUntil: nextYear.toISOString() });
                                                await fetchUserData();
                                            }
                                        }}
                                        className="w-full px-4 py-2 text-xs font-bold text-slate-600 bg-slate-200 rounded-md hover:bg-slate-300 flex justify-center items-center"
                                    >
                                        Grant 1 Year Access (Unlock)
                                    </button>
                                </div>
                            )}

                            <button onClick={handleSimulatePayment} disabled={isDisabled} className="mt-4 w-full px-4 py-2 text-sm font-medium text-white bg-red-500 rounded-md hover:bg-red-600 flex justify-center items-center min-h-[36px] disabled:opacity-50">
